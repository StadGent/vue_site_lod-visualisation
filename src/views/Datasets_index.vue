    ...
    <section class="overview-layout">
      ...
      <div class="cta-block highlight highlight--left link" id="ODP_CTA">
        <div class="highlight__inner">
          ...
          <p>
            Ga naar het
            <a href="https://data.stad.gent/">Stad Gent Open Data Portaal</a>
            voor een overzicht van alle open data die Stad Gent ter beschikking stelt, bijv. ook open data die in een csv- of kml-formaat gepubliceerd wordt.
          </p>
          <div class="links">
        ...
        >
          <article class="teaser-content">
            <div class="content__second">
              ...
              <p>{{ $filters.truncate(dataset.description.value) }}</p>
              <router-link
                :to="{ name: 'datasets_show', params: { id: dataset.btoa } }"
                class="read-more standalone-link"
              >
                Lees meer
                <span class="visually-hidden">{{ dataset.title.value }}</span>
              </router-link>
            </div>
          </article>
          <router-link
      ...
      <Pagination :total="totalPages" :active="currentPage" />
    </section>
  </transition>
</template>
  ...
  computed: {
    ...
    filteredSets() {
      return this.datasets.filter(
        ({ title, description }) =>
          this.checkValues(this.searchValueArray, title.value + ' ' + description.value)
      );
    },
    totalPages() {
